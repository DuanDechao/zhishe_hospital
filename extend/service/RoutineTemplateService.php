...
class RoutineTemplateService{
    ...
    //订单支付成功
    const ORDER_PAY_SUCCESS = '98';
    //预约成功
    const ORDER_SUB_SUCCESS = '2511';
    //预约修改成功
    const ORDER_SUB_MODIFY_SUCCESS = '5656';
    //砍价成功
    const BARGAIN_SUCCESS = 'AT1173';
    //申请退款通知
    const ORDER_REFUND_STATUS = 'AT0036';
    //退款成功
    const ORDER_REFUND_SUCCESS = '3177';
    //订单发货提醒(快递)
    ...
    public static function sendTemplate($openId = '',$templateId = '',$link = '',$dataKey = array(),$formId = '',$emphasisKeyword = ''){
        ...
        try{
            ...
            $ret = json_decode(RoutineServer::curlPost($url,json_encode($data)),true);
//            echo var_export($ret);
            return $ret;
        }catch (\Exception $e){
            return false;

...
class StoreProduct extends AuthController
{
    ...
    public function index()
    {
        ...
        //获取分类
        $this->assign('cate',HospitalModel::getTierList());
        //出售中产品
...
//        return $this->fetch('public/common_form');
        ...
        $field = [
            ...
            Form::input('store_name','套餐名称')->col(Form::col(24)),
            Form::input('store_info','套餐简介')->type('textarea'),
            Form::input('keyword','套餐关键字')->placeholder('多个用英文状态下的逗号隔开'),
            Form::input('unit_name','套餐单位','件'),
            Form::select('type', '套餐类型')->setOptions(function(){
                $list = ProductModel::getProductTypesConfig();
                $menus=[];
                foreach ($list as $menu) {
            ...
            Form::frameImageOne('image','套餐主图片(305*305px)',Url::build('admin/widget.images/index',array('fodder'=>'image')))->icon('image')->width('100%')->height('550px')->spin(0),
            Form::frameImages('slider_image','套餐轮播图(640*640px)',Url::build('admin/widget.images/index',array('fodder'=>'slider_image')))->maxLength(5)->icon('images')->width('100%')->height('550px')->spin(0),
			//Form::select('price_type','定价类型')->options($metalLabels)->col(8),
            Form::number('price','售价')->min(0)->col(8),
            Form::number('ot_price','市场价')->min(0)->col(8),
           ...
           // Form::number('metal_weight','金器克数')->min(0)->col(8),
            ...
            Form::radio('is_show','套餐状态',0)->options([['label'=>'上线','value'=>1],['label'=>'下线','value'=>0]])->col(8),
        ...
        $form->setMethod('post')->setTitle('添加套餐')->components($field)->setSuccessScript('parent.$(".J_iframe:visible")[0].contentWindow.location.reload();');
    ...
    public function save(Request $request)
    {
        ...
        if(!$data['cate_id']) return Json::fail('请选择医院');
        if(!$data['type']) return Json::fail('请选择套餐类型');
        if(!$data['store_name']) return Json::fail('请输入套餐名称');
        if(count($data['image'])<1) return Json::fail('请上传套餐图片');
        if($data['cost'] == '' || $data['ot_price'] < 0) return Json::fail('请输入套餐成本价');
        if($data['sales'] == '' || $data['sales'] < 0) return Json::fail('请输入预约量');
    ...
    public function edit_content1($id){
        if(!$id) return $this->failed('数据不存在');
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>ProductModel::where('id',$id)->value('description1'),
            'field'=>'description1',
            'action'=>Url::build('change_field',['id'=>$id,'field'=>'description1'])
        ]);
        return $this->fetch('public/edit_content');
    }
    public function edit_content2($id){
        if(!$id) return $this->failed('数据不存在');
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>ProductModel::where('id',$id)->value('description2'),
            'field'=>'description2',
            'action'=>Url::build('change_field',['id'=>$id,'field'=>'description2'])
        ]);
        return $this->fetch('public/edit_content');
    }
    public function edit_content3($id){
        if(!$id) return $this->failed('数据不存在');
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>ProductModel::where('id',$id)->value('description3'),
            'field'=>'description3',
            'action'=>Url::build('change_field',['id'=>$id,'field'=>'description3'])
        ]);
        return $this->fetch('public/edit_content');
    }
    public function edit_content4($id){
        if(!$id) return $this->failed('数据不存在');
        $product = ProductModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $this->assign([
            'content'=>ProductModel::where('id',$id)->value('description4'),
            'field'=>'description4',
            'action'=>Url::build('change_field',['id'=>$id,'field'=>'description4'])
        ]);
        return $this->fetch('public/edit_content');
    }
    ...
    public function edit($id)
    {
        ...
        $form = Form::create(Url::build('update',array('id'=>$id)),[
            ...
            Form::input('store_name','套餐名称',$product->getData('store_name')),
            Form::input('store_info','套餐简介',$product->getData('store_info'))->type('textarea'),
            Form::input('keyword','套餐关键字',$product->getData('keyword'))->placeholder('多个用英文状态下的逗号隔开'),
            Form::input('unit_name','套餐单位',$product->getData('unit_name')),
            Form::select('type', '套餐类型', $product->getData('type'))->setOptions(function(){
                $list = ProductModel::getProductTypesConfig();
                $menus=[];
                foreach ($list as $menu) {
                    $menus[] = ['value' => $menu, 'label'=>$menu];
            ...
            Form::frameImageOne('image','套餐主图片(305*305px)',Url::build('admin/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image')->width('100%')->height('550px')->spin(0),
            Form::frameImages('slider_image','套餐轮播图(640*640px)',Url::build('admin/widget.images/index',array('fodder'=>'slider_image')), $sliderImage)->maxLength(5)->icon('images')->width('100%')->height('550px')->spin(0),
            Form::number('price','售价',$product->getData('price'))->min(0)->precision(2)->col(8),
            Form::number('ot_price','市场价',$product->getData('ot_price'))->min(0)->col(8),
            Form::number('sales','预约量',$product->getData('sales'))->min(0)->precision(0)->col(8),
            Form::number('ficti','虚拟预约',$product->getData('ficti'))->min(0)->precision(0)->col(8),
            Form::number('cost','成本价',$product->getData('cost'))->min(0)->col(8),
			Form::number('stock', '库存', $product->getData('stock'))->min(0)->col(8),
            Form::radio('is_show','产品状态',$product->getData('is_show'))->options([['label'=>'上架','value'=>1],['label'=>'下架','value'=>0]])->col(8),
        ]);
        $form->setMethod('post')->setTitle('编辑套餐')->setSuccessScript('parent.$(".J_iframe:visible")[0].contentWindow.location.reload();');
        $this->assign(compact('form'));
        return $this->fetch('public/form-builder');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(!$data['cate_id']) return Json::fail('请选择所属医院');
        if(!$data['type']) return Json::fail('请选择套餐类型');
        if(!$data['store_name']) return Json::fail('请输入套餐名称');
//        if(!$data['store_info']) return Json::fail('请输入产品简介');
//        if(!$data['keyword']) return Json::fail('请输入产品关键字');
        if(count($data['image'])<1) return Json::fail('请上传套餐图片');
        if(count($data['slider_image'])<1) return Json::fail('请上传套餐轮播图');
        if(count($data['slider_image'])>5) return Json::fail('轮播图最多5张图');
		if($data['price'] == '' || $data['price'] < 0) return Json::fail('请输入套餐售价');
		if($data['ot_price'] == '' || $data['ot_price'] < 0) return Json::fail('请输入套餐市场价');
        if($data['cost'] == '' || $data['cost'] < 0) return Json::fail('请输入套餐成本价');

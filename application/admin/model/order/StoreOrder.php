...
//        $data=($data=$model->page((int)$where['page'],(int)$where['limit'])->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            ...
            }else if($item['paid']==1 && $item['confirmed']==0 && $item['status']==0 && $item['refund_status']==0){
                $item['status_name']='待预约';
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==0 && $item['refund_status']==0){
                $item['status_name']='待体检';
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==1 && $item['delivery_type']=='express' && $item['refund_status']==0){
                $item['status_name']='报告单已寄出';
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==1 && $item['delivery_type']=='send' && $item['refund_status']==0){
                $item['status_name']='报告单待签收';
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==2 && $item['refund_status']==0){
                $item['status_name']='待评价';
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==3 && $item['refund_status']==0){
...
HTML;
            ...
            if($item['paid']==0 && $item['status']==0 && $item['refund_status']==0){
                ...
                $item['_status']=3;
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==1 && $item['refund_status']==0){
                $item['_status']=4;
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==2 && $item['refund_status']==0){
                $item['_status']=5;
            }else if($item['paid']==1 && $item['confirmed']==1 && $item['status']==3 && $item['refund_status']==0){
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function sendOrderGoods($oid = 0,$postageData=array()){
        ...
        if($postageData['delivery_type'] == 'send'){//送货
             ...
             */
            //RoutineTemplate::sendSubscribeModifySuccess('', $order['order_id']);
        }
    }
    ...
    public static function refundOrderGoods($oid = 0,$postageData=array()){
        if(!$oid || !$postageData) return true;
        $order = self::where('id',$oid)->find();
        RoutineTemplate::sendOrderRefundSuccess($order, $postageData);
	}

...
class StoreOrder extends ModelBasic
{
    ...
    public static function cacheKeyCreateOrder($uid,$key,$addressId,$payType,$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0,$vip_level=0,
        $useCondBounty=false,$useFullBounty=false, $subDate = 0, $realName='',$sex='', $married='', $cardType='',$cardNumber='', $phoneNumber='', $addressRegion='', $addresStress='')
    {
		...
		*/
        ...
        $orderInfo = [
            ...
            'report_address'=>$addressRegion.' '.$addresStress,
        ];
...
//            HookService::listen('yue_pay_product',$userInfo,$orderInfo,false,PaymentBehavior::class);
    ...
    public static function paySuccess($orderId,$formId = '')
    {
		...
		RoutineTemplate::sendSubscribeSuccess($formId,$orderId);
...
//        ],Url::build('My/order',['uni'=>$order['order_id']],true,true));
    ...
    public static function tidyOrder($order,$detail = false)
    {
        if($detail == true && isset($order['id'])){
            ...
            foreach ($cartInfo as $k=>$cart){
                ...
                $cartInfo[$k]['unique'] = $k;
                $cartInfo[$k]['productInfo']['hospital_name'] = StoreProduct::getHospitalName($cartInfo[$k]['product_id']);
            }
        ...
        if(!$order['paid'] && $order['pay_type'] == 'offline' && !$order['status'] >= 2){
            ...
            if($order['pink_id']){
                if(StorePink::where('id',$order['pink_id'])->where('status',1)->count()){
                    ...
                    $status['_type'] = 1;
                    $status['_title'] = '待预约';
                    $status['_msg'] = '管理员正在确认预约日期,请耐心等待';
                    $status['_class'] = 'state-nfh';
                }
            }else if($order['confirmed'] == 0){
                $status['_type'] = 1;
                $status['_title'] = '待预约';
                $status['_msg'] = '管理员正在确认预约日期,请耐心等待';
                $status['_class'] = 'state-nfh';
            }else if ($order['confirmed'] == 1) {
               $status['_type'] = 5;
               $status['_title'] = '待体检';
               $status['_msg'] = '您的预约日期:'.$order['sub_date'].', 请您准时参加体检';
               $status['_class'] = 'state-ysh';
            }
        }else if($order['status'] == 1){
            $status['_type'] = 2;
            $status['_title'] = '待收报告单';
            $status['_msg'] = '您的报告单正在派送中，请耐心等待';
            $status['_class'] = 'state-ysh';
        }else if($order['status'] == 2){
            $status['_type'] = 3;
            $status['_title'] = '待评价';
            $status['_msg'] = '已体检结束,快去评价一下吧';
    ...
    public static function statusByWhere($status,$model = null)
    {
        ...
        if('' === $status)
            ...
            return $model->where('paid',1)->where('order_id','NOT IN',implode(',',$orderId))->where('confirmed', 0)->where('status',0)->where('refund_status',0);
        else if($status == 2)
            ...
            return $model->where('paid',1)->where('confirmed',1)->where('status', 0)->where('refund_status',0);
        else if($status == -1)
    ...
    public static function getUserOrderList($uid,$status = '',$first = 0,$limit = 8)
    {
        $list = self::statusByWhere($status)->where('is_del',0)->where('uid',$uid)
            ->field('sub_date,confirmed,real_name,seckill_id,bargain_id,combination_id,id,order_id,pay_price,total_num,total_price,pay_postage,total_postage,paid,status,refund_status,pay_type,coupon_price,deduction_price,pink_id,delivery_type,unique')
    ...
    public static function getUserReportList($uid,$first = 0,$limit = 8)
    {
        $list = self::where('is_upload_report', 1)->where('is_del',0)->where('uid',$uid)
            ->field('sub_date,confirmed,upload_report_time,real_name,seckill_id,bargain_id,combination_id,id,order_id,pay_price,total_num,total_price,pay_postage,total_postage,paid,status,refund_status,pay_type,coupon_price,deduction_price,pink_id,delivery_type,unique')
            ->order('add_time DESC')->limit($first,$limit)->select()->toArray();
        foreach ($list as $k=>$order){
            $list[$k] = self::tidyOrder($order,true);
        }
    ...
    public static function getReportDetail($uid, $id) {
        return self::where('uid', $uid)->where('id', $id)->field('order_id, real_name, upload_report_time, report_card')->select()->toArray();
    }

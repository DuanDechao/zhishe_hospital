...
class UserExtract extends ModelBasic
{
    ...
    public static function userExtract($userInfo,$data){
        ...
        $balance = 0;
        if ($data['moneyType'] == 'yue') {
          if($data['money'] > $userInfo['now_money']) return self::setErrorInfo('余额不足');;
          $balance = bcsub($userInfo['now_money'],$data['money'],2);
        } else if ($data['moneyType'] == 'extra') {
          if($data['money'] > $userInfo['distribution_money']) return self::setErrorInfo('余额不足');;
          $balance = bcsub($userInfo['distribution_money'],$data['money'],2);
        }
        $insertData = [
            'uid'=>$userInfo['uid'],
        ...
        if ($data['moneyType'] == 'yue') {
          ...
          $res2 = User::edit(['distribution_money'=>$balance],$userInfo['uid'],'uid');
          $res3 = UserBill::expend('分销奖励金提现',$userInfo['uid'],'distribution_money','extract',$data['money'],$res1['id'],$balance,$mark);
          $res = $res2 && $res3;
        }
        self::checkTrans($res);
        if($res){

...
//            unset($productAttr[$k]['attr_values']);
		...
		$data['hospitalName'] = StoreHospital::getName($storeInfo['cate_id']);
...
//            HookService::afterListen('store_product_set_cart_after',$res,$this->userInfo,false,StoreProductBehavior::class);
    ...
    public function create_order($key = '')
    {
        ...
        list($addressId,$couponId,$payType,$useIntegral,$mark,$combinationId,$pinkId,$seckill_id,$formId,$bargainId,$useCondBounty,$useFullBounty, $subDate, $realName, $sex, $married, $cardType, $cardNumber, $phoneNumber, $addressRegion, $addressStress) = UtilService::postMore([
            'addressId','couponId','payType','useIntegral','mark',['combinationId',0],['pinkId',0],['seckill_id',0],['formId',''],['bargainId',''], 'useCondBounty','useFullBounty','subDate', 'realName', 'sex', 'married', 'cardType', 'cardNumber', 'phoneNumber', ['addressRegion',''], ['addressStress','']],Request::instance(),true);
        ...
        $order = StoreOrder::cacheKeyCreateOrder($this->userInfo['uid'],$key,$addressId,$payType,$useIntegral,$couponId,$mark,$combinationId,$pinkId,$seckill_id,$bargainId,$this->userInfo['level'],$useCondBounty,$useFullBounty, $subDate, $realName, $sex, $married, $cardType, $cardNumber, $phoneNumber, $addressRegion, $addressStress);
...
//        StoreOrder::delCombination();//删除拼团未支付订单
     ...
     * @return \think\response\Json
     */
    public function get_user_report_list($first = 0, $limit = 8)
    {
//        StoreOrder::delCombination();//删除拼团未支付订单
        $list = StoreOrder::getUserReportList($this->userInfo['uid'],$first,$limit);
        foreach ($list as $k=>$order){
            $list[$k] = StoreOrder::tidyOrder($order,true);
            if($list[$k]['_status']['_type'] == 3){
                foreach ($order['cartInfo']?:[] as $key=>$product){
    ...
    public function spread_uid(Request $request){
        ...
        if($data['spread_uid']){
            if(!$this->userInfo['spread_uid']){
				...
				if($res){
					UserBounty::giveUserBindSpreaderBounty($this->userInfo['uid'], $data['spread_uid']);
...
//        return JsonService::successful($posterPath);
    ...
    public function report_detail($id = 0) {
        $list = StoreOrder::getReportDetail($this->userInfo['uid'], $id);
        return JsonService::successful($list);

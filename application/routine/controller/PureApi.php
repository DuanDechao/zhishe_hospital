...
use app\routine\model\address\AddressCity;
 ...
 *
 */
class PureApi extends Controller{
    ...
    public function get_product_list(Request $request)
    {
       $data = UtilService::getMore([
		...
		if($hospital_id != 0) $model->where('cate_id', $hospital_id);
    //    if($city != '') $model->where('city', $city);
       // if($type != '') $model->where('type', $type);
		$model->field('ficti + sales as show_sales');
        $baseOrder = '';
        if($priceOrder) {
            $baseOrder .= $priceOrder == 'desc' ? 'price DESC' : 'price ASC';
            $baseOrder .= ', ';
        }
        if($salesOrder) {
            $baseOrder .= $salesOrder == 'desc' ? 'show_sales DESC' : 'show_sales ASC';//虚拟和真实销量
            $baseOrder .= ', ';
        }
        Log::write(var_export($baseOrder, true), "qqqqqqqq");
        $model->order($baseOrder.'sort DESC, add_time DESC');
        $list = $model->limit($first,$limit)->field('id,store_name,cate_id,image,sales,ficti,price,stock,keyword')->select()->toArray();
        return JsonService::successful($list);
    }
    public function get_hospital_list(Request $request)
    {
       $data = UtilService::getMore([
        ...
        $cate_ids = [];
        if($data['productType'] != ''){
            $hospitals = StoreProduct::getProductHospital($data['productType']);
            $cate_ids = array_keys($hospitals);
        }
        $model = StoreHospital::validWhere();
        if (count($cate_ids) > 0) {
            $model->where('id', 'IN', $cate_ids);
        ...
        if(!empty($keyword)) $model->where('keyword|store_name','LIKE',htmlspecialchars("%$keyword%"));
        if($news!=0) $model->where('is_new',1);
        //$model->where('city', 'LIKE', $data['city']);
        if($data['region'] != '') $model->where('region', 'LIKE', $data['region']);
        if($data['hospitalLevel'] != '') $model->where('hospital_type', 'LIKE', $data['hospitalLevel']);
		$model->field('ficti + sales as show_sales');
        $baseOrder = '';
        if($salesOrder) $baseOrder = $salesOrder == 'desc' ? 'show_sales DESC' : 'show_sales ASC';//虚拟和真实销量
        if($baseOrder) $baseOrder .= ', ';
        $model->order($baseOrder.'add_time DESC');
        $list = $model->limit($first,$limit)->field('id,store_name,image,sales,ficti,region,hospital_type,keyword,latitude, longitude')->select()->toArray();
        return JsonService::successful($list);
    }
    /**
     * 购物车
    ...
    public function details(Request $request){
        ...
        $id = $data['id'];
        Log::write(var_export($id, true), "ddddddqqqqqqqq");
...
//            unset($productAttr[$k]['attr_values']);
        ...
        Log::write(var_export($storeInfo, true), "ddddddddd");
        $data['storeInfo'] = $storeInfo;
        $hospitalBaseInfo = StoreHospital::getBaseInfo($storeInfo['cate_id']);
        if (count($hospitalBaseInfo) > 0 ) {
            $data['hospitalName'] = $hospitalBaseInfo[0]['store_name'];
            $data['latitude'] = $hospitalBaseInfo[0]['latitude'];
            $data['longitude'] = $hospitalBaseInfo[0]['longitude'];
        }
    ...
    public function hospital_details(Request $request){
        $data = UtilService::postMore(['id'],$request);
        $id = $data['id'];
        if(!$id || !($storeInfo = StoreHospital::getValidProduct($id))) return JsonService::fail('商品不存在或已下架');
        $data['storeInfo'] = $storeInfo;
        return JsonService::successful($data);
    }
...
//    }
    ...
    public function get_cid_article($cid = 0,$first = 0,$limit = 8){
        $list = ArticleModel::cidByArticleList($cid,$first,$limit,'id,title,image_input,visit,add_time,synopsis,url,is_hot,is_banner')?:[];
...
//        return JsonService::successful($posterPath);
    ...
    public function get_hospital_levels_config(){
		$hospitalLevelsConfig = StoreHospital::getHospitalLevelConfig();
        return JsonService::successful($hospitalLevelsConfig);
    }

    public function get_product_types_config() {
        $productTypesConfig = StoreHospital::getProductTypesConfig();
        return JsonService::successful($productTypesConfig);
    }

    public function get_city_list() {
        /*$res = [];
        for ($id  = 1; $id < 200; $id++) {
        $city_list = "https://douban.uieee.com/v2/loc/list?start=".$id."&count=100";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $city_list);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $result = curl_exec($ch);
        curl_close($ch);
        $citys = json_decode($result, true);
        foreach ($citys["locs"] as $city) {
            $res[$city['id']] = $city;
        }
        }

        foreach($res as $city) {
            AddressCity::setCityInfo($city);
        }

        LOG::write(var_export($res, true), "get_city_list aaaaaaaaaa");
         */
        $list = AddressCity::getCityList();
        return JsonService::successful($list);
    }

    public function get_product_name_list() {
        $list = StoreProduct::getProductNameList();
        LOG::write(var_export($list, true), "get_city_listsssssssss");
        return JsonService::successful($list);
    }

    public function send_templete_msg() {
      RoutineTemplate::sendOrderSuccess("","wx2020011122440310006");//发送模板消息
      RoutineTemplate::sendSubscribeSuccess("","wx2020011122440310006");//发送模板消息
      RoutineTemplate::sendSubscribeModifySuccess("","wx2020011122440310006");//发送模板消息
      $order = StoreOrder::where('order_id',"wx2020011122440310006")->find();
      RoutineTemplate::sendOrderRefundSuccess($order);//发送模板消息
    }

    public function test_refund() {
      RoutineRefund::doRefund(0.01,0.01,'wx2020011419260510001','','wx2020011419260510001', '');
    }
